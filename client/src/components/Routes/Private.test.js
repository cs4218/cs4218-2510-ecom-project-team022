...
describe("PrivateRoute", () => {
  ...
  it("renders <Spinner /> when user logged in with an invalid token", async () => {
    mockUseAuth.mockReturnValue([{ user: { name: "notadmin", role: 0 }, token: "invalid-token" }, jest.fn()]);
  ...
  it('renders <Outlet /> when user is logged in', async () => {
    ...
    mockUseAuth.mockReturnValue([{ user: { name: "user", role: 0 }, token: "valid-user-token" }, jest.fn()]);
