...
const defaultUser = { user: { name: "Test User", email: "test@example.com", phone: "1234567890", address: "123 Test St" }, token: "token" }
...
describe("Profile page", () => {
  ...
  it("renders the form with correct initial values from auth context", () => {
    ...
    expect(screen.getByPlaceholderText("Enter Your Address").value).toBe("123 Test St");
  ...
  it("submits the form and updates context/localStorage on success", async () => {
    axios.put.mockResolvedValueOnce({ data: { updatedUser: { name: "New Name", email: "test@example.com", phone: "1112223333", address: "New Address" } } });
    render(<Profile />);
    ...
    fireEvent.change(screen.getByPlaceholderText("Enter Your Address"), { target: { value: "New Address" } });
    fireEvent.click(screen.getByText("UPDATE"));
    await waitFor(() => expect(axios.put).toHaveBeenCalled());
    expect(mockSetAuth).toHaveBeenCalledWith(expect.objectContaining({ user: expect.objectContaining({ name: "New Name" }) }));
